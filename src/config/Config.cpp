...
void Config::parse_server(std::vector<std::string> &tokens, ServerConf &server, size_t line) {
	...
	if (tokens.size() < 3 || tokens[tokens.size() - 1] != ";") {
		...
		if (!is_num(tokens[1]) || std::atoi(tokens[1].c_str()) < 1 ||
		    std::atoi(tokens[1].c_str()) > 599) {
		...
		if (tokens.size() != 3) {
			setError(line,
				 "Invalid max size syntax, expected 'client_max_body_size SIZE ;'");
			return;
		}
	...
	if (tokens.size() < 3 || tokens[tokens.size() - 1] != ";") {
		...
		for (size_t i = 1; (i + 1) < tokens.size(); ++i) {
			...
			if (tokens[i] != "GET" && tokens[i] != "POST" && tokens[i] != "DELETE") {
				setError(line,
					 "Invalid method, allowed methods: GET, POST, DELETE");
				return;
			}
		}
	} else if (key == "return") {
		...
		if (location.redirect_status < 300 || location.redirect_status > 308 ||
		    !is_num(tokens[1])) {
		...
		if (tokens.size() != 3) {
			setError(line, "Invalid cgi syntax, expected 'cgi_max_output SIZE ;'");
			return;
		}
		int val = std::atoi(tokens[1].c_str());
		if (val < 0 || !is_num(tokens[1])) {
			setError(line, "Invalid cgi max output value, expected positive number");
			return;
		}
		...
		int val = std::atoi(tokens[1].c_str());
		if (val < 0 || !is_num(tokens[1])) {
			setError(line, "Invalid cgi timeout value, expected positive number");
			return;
		}
		location.cgi_timeout_ms = val;
	} else if (key == "client_max_body_size") {
		if (tokens.size() != 3) {
			setError(line,
				 "Invalid max size syntax, expected 'client_max_body_size SIZE ;'");
