...
// client_max_body_size(1 << 20) {} 	ServerConfig() 	    : root("www_giulio/"),
// index("index.html"), client_max_body_size(1 << 20), 	      cgi_timeout_ms(5000),
...
struct LocationConf {
	...
	LocationConf()
	    : redirect_enabled(false), redirect_status(0), has_root(false), has_index(false),
	      ...
	      has_php(false), cgi_timeout_ms(5000), cgi_maxOutput(1024 * 1024), has_max_size(false),
	      max_size(1024 * 1024) {}
};
