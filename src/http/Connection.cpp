...
Connection::Connection(const ServerConf &cfg)
    : headers_done(false), responded(false), peer_closed(false), close_after(false),
      state(READING_HEADERS), want_body(0), is_chunked(false), res(200), file_fd(-1),
...
void Connection::printStatus(const std::string &label) {
	std::string ConnStateStr[CLOSING + 1] = {"READING_HEADER",   "READING_BODY",
						 "READY_TO_RESPOND", "WRITTING_RESPONSE",
						 "WAITING",	     "CLOSING"};
	Logger::simple("%s - in: %lu bytes, out: %lu bytes, last: %s state: %s [%d]", label.c_str(),
		       (unsigned long)in.size(), (unsigned long)out.size(),
		       formatTime(last_active).c_str(), ConnStateStr[(int)state].c_str(),
		       (int)state);
}
