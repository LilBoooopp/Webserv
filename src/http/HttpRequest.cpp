...
void HttpRequest::log() const {
  ...
  const char *methodClr = method == "GET"      ? YELLOW
                          ...
                          : method == "DELETE" ? rgb(110, 47, 193)
                                               : GREEN;
  Logger::request("%sMethod %s%s%s%s, %starget %s%s, %sversion%s %s", GREY,
                  methodClr, BLD, method.c_str(), TS, GREY, URLCLR,
                  target.c_str(), GREY, TS, version.c_str());
...
bool HttpRequest::eraseHeader(const std::string &key) {
...
void HttpRequest::removeBoundary(std::string &body,
