...
HttpResponse::HttpResponse(int statusCode, const std::string &version)
    : version_(version), statusCode_(statusCode), reasonPhrase_(reasonForStatus(statusCode)),
      body_("") {
...
void HttpResponse::setContentType(const std::string &type) { setHeader("Content-Type", type); }
...
static std::string getColorCode(int code) {
	return (code < 200 ? BLUE : code < 300 ? GREEN : code < 400 ? YELLOW : RED);
}
...
void HttpResponse::printResponse(int fd) {
	...
	Logger::response("at fd %d: %s%s%s %s", fd, getColorCode(getStatus()).c_str(),
