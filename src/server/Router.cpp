...
const LocationConf *Router::matchLocation(const ServerConf &conf, const std::string &path) {
...
void Router::loadErrorPage(Connection &c) {
	...
	if (it != pages.end()) {
		...
		// Check if it's HTML content (default error page) or a file path
		if (content.find("<!DOCTYPE") == 0 || content.find("<html") == 0) {
			// It's inline HTML content
			c.res.setBody(content);
			c.res.setContentType("text/html");
		} else {
			// It's a file path - load from filesystem
			const ServerConf &srv = c.cfg;
			std::string fullPath = safe_join_under_root(srv.root, content);
			...
			if (fd >= 0) {
				...
				Logger::response(
				    "error_page configured for %d but file '%s' not found",
