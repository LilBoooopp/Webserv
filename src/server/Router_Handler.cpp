...
std::string buildDirListing(const std::string &root, const std::string &uri) {
  ...
  std::string prev = uri;
  if (prev.size() > 1 && prev[prev.size() - 1] == '/')
  ...
  if (dir) {
    ...
    while ((entry = readdir(dir)) != NULL) {
      ...
      html << "<tr><td><a href=\"" << href << "\">" << showName
           << "</a></td><td>" << fileType << "</td><td>" << mtime_str
           << "</td><td>" << sizeStr << "</td></tr>\n";
    }
    closedir(dir);
  }
...
static void respondWithDirListing(Connection &c) {
  ...
  if (c.req.method == "HEAD") {
    ...
    c.res.setHeader("Content-Length",
                    size_to_str(static_cast<off_t>(dirHtml.size())));
  } else {
    c.res.setStatus(200, "OK");
...
void Router::handle(Connection &c) {
  ...
  Logger::router("%srouted %s%s%s -> %s%s", GREY, URLCLR, c.req.target.c_str(),
                 GREY, URLCLR, path.c_str());
    ...
    if (is_dir(st)) {
      ...
      if (!index_files->empty()) {
        ...
        if (file_exists(idx) && is_reg(idx)) {
          ...
          if (is_head) {
            c.res.setStatus(200, "OK");
            c.res.setHeader("Content_Length", size_to_str(st.st_size));
            return;
          }
          ...
          if (c.loc && c.loc->autoindex) {
            Logger::response("index \'%s\' missing -> dir listing",
                             idx.c_str());
            respondWithDirListing(c);
            return;
          }
          // Index file configured but doesn't exist → 404
          Logger::response("index \'%s\' missing -> 404", idx.c_str());
