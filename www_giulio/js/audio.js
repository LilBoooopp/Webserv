function initAudioPlayer(p = [100, 50]) {
	...
	function addDeleteButton() {
		var button = addButton("X", [p[0] + 60, p[1] + 40], () => {
			if (!mus.isUploaded) {
				return;
			}
			fetch(`/delete`, {
	...
	loadUploadedAudio();
	var delBtn = addDeleteButton();

	const label = addDiv(trackNames[0], [p[0] - 40, p[1] + 50], null, null, null, false);
	label.style.transform = "scale(0.8)";
	label.style.transformOrigin = "0 0";
	const artistLabel = addDiv(artits[0], [p[0] - 40, p[1] + 65], null, null, null, false);
	...
	function moveHandle(e) {
		...
		const ratio = (e.clientX - rect.left) / rect.width;
		const clamped = Math.max(0, Math.min(0.92, ratio));
	...
	var AudioLevelBackground = writeBox(80, 5, p[0] - 38, p[1] + 80, "black");
	var fillbackground = writeBox(80, 5, p[0] - 38, p[1] + 80, "white");
	...
	const button = addButton("Upload", [p[0], p[1] + 120], () => openFileDialog(uploadFile), null, "rgba(255, 255, 255, 0.24)");
